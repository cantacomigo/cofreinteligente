...
const FinancialHealthScore: React.FC<FinancialHealthScoreProps> = ({ score }) => {
  ...
  const label = score >= 80 ? { text: 'Excelente', icon: <ShieldCheck className="w-3 h-3 text-emerald-500" /> } :
                score >= 50 ? { text: 'Regular', icon: <TrendingUp className="w-3 h-3 text-amber-500" /> } :
                { text: 'Crítica', icon: <AlertCircle className="w-3 h-3 text-rose-500" /> };
